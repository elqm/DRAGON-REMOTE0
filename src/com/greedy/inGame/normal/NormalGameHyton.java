...
public class NormalGameHyton extends JPanel implements ActionListener {
...
//		JButton preferencesBt = new JButton();
	...
	private void initGame() {
		...
		characterY = HEIGHT - characterImage.getHeight(null) - 90; // 캐릭터 시작 지점 y축 위치
		for(int i = 0; i < weaponImageArr.length; i++) {
	...
	private void moveWeapon() {
		for(int i = 0; i < weaponImageArr.length; i++) {
			if(weaponYArr[i] > 8) {
				weaponYArr[i] += 8; // 무기 속도 조절
			} else {
				weaponYArr[i] += (4 + i); // 무기 속도 조절
			}
